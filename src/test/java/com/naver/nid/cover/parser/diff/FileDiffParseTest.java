...
class FileDiffParseTest {
	...
	public void parserSimpleTest() {
		...
		FileRawDiffReader reader = new FileRawDiffReader(new BufferedReader(new StringReader(s)));
		DiffParser parser = new DiffParser();
		Stream<Diff> parse = parser.parse(reader);
	...
	public void parserComplexTest() throws URISyntaxException, FileNotFoundException {
		...
		FileRawDiffReader reader = new FileRawDiffReader(new BufferedReader(new FileReader(new File(diffUrl.toURI()))));
		DiffParser parser = new DiffParser();
		Stream<Diff> parse = parser.parse(reader);
