...
@Slf4j
public abstract class DiffParser implements Iterator<RawDiff> {
	...
	public Stream<Diff> parse() {
		return StreamSupport.stream(this.toIterable().spliterator(), false).map(diffMapper);
	}

	private Iterable<RawDiff> toIterable() {
		return () -> this;
	}
}
